...
: m_rows(nRows), m_cols(nCols), m_player(nullptr), m_nFlatulans(0), m_history(nRows, nCols)   // initialize m_history also!
{
...
{
    //When a Flatulan is preached to but not converted, its City's history object (m_history) must be notified about the position it was at when preached to???
    return m_history;
}
