...
void testDecrypter()
{
	...
	vector<string> v1 = d.crack("smxsdg SGOSDG gfvgx!!");
    vector<string> v2 = d.crack("SGOSDG smxsdg gfvgx!!");
    ...
    assert(v1.size() == 4  &&  equal(v1.begin(), v1.end(), expected1));
	assert(v2.size() == 4  &&  equal(v2.begin(), v2.end(), expected2));
	assert(d.crack("smxsdg SGOSDG gfvgy!!").empty());
    assert(d.crack("smxsdg SGOSDG@ gfvgx!!").empty());
	remove(FILENAME);
}
